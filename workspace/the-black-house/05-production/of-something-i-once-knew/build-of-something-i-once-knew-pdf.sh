...
CACHE_HOME="${TMPDIR:-/tmp}/of-something-i-once-knew-lualatex-home"

mkdir -p "$CACHE_HOME/.cache" "$CACHE_HOME/.texlive2025/texmf-var"

pandoc \
  "$ROOT_DIR/of-something-i-once-knew.md" \
  --standalone \
  --to=latex \
  ...
  --metadata title="Of Something I Once Knew" \
  --metadata author="Francis Dove" \
  -o "$ROOT_DIR/of-something-i-once-knew.tex"

HOME="$CACHE_HOME" \
TEXMFCACHE="$CACHE_HOME/.texlive2025/texmf-var" \
lualatex -interaction=nonstopmode -output-directory="$ROOT_DIR" "$ROOT_DIR/of-something-i-once-knew.tex"
