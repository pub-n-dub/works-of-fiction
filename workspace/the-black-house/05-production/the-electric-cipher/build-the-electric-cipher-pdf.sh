...
CACHE_HOME="${TMPDIR:-/tmp}/the-electric-cipher-lualatex-home"

mkdir -p "$CACHE_HOME/.cache" "$CACHE_HOME/.texlive2025/texmf-var"

pandoc \
  "$ROOT_DIR/the-electric-cipher.md" \
  --standalone \
  --to=latex \
  ...
  --metadata title="The Electric Cipher" \
  --metadata author="Francis Dove" \
  -o "$ROOT_DIR/the-electric-cipher.tex"

HOME="$CACHE_HOME" \
TEXMFCACHE="$CACHE_HOME/.texlive2025/texmf-var" \
lualatex -interaction=nonstopmode -output-directory="$ROOT_DIR" "$ROOT_DIR/the-electric-cipher.tex"
