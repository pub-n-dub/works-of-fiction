...
This file records the differences introduced when `The Electric Cipher` was extracted into standalone output files and then normalized to match the rewrite standard used for `Prelude`.

## Current State

- `the-electric-cipher-original.md` preserves the archival extract from the working manuscript
- `the-electric-cipher-edited.md` is the standalone editorial version
- `the-electric-cipher.md` matches the current edited build source used for PDF generation
...
Standalone PDF generation is handled through `build-the-electric-cipher-pdf.sh`, which uses a writable LuaLaTeX cache path for the current environment.
