...
This file records the differences introduced when `The Itch` was extracted into standalone output files and then normalized to match the rewrite standard used for `Prelude`.

## Current State

- `the-itch-original.md` preserves the archival extract from the working manuscript
- `the-itch-edited.md` is the standalone editorial version
- `the-itch.md` matches the current edited build source used for PDF generation
...
Standalone PDF generation is handled through `build-the-itch-pdf.sh`, which uses a writable LuaLaTeX cache path for the current environment.
